...
def run_labeling_and_splitting():
    logging.info("Source Labeling and Data Splitting")
    try:
        df = pd.read_csv(config.DATA_FOR_ML_FILE)
    except FileNotFoundError:
        logging.error("Input file not found. Run 02_data_processing.py first.")
        return
    ...
    try:
        ...
        logging.error(f"Error splitting data: {e}. Dataset might be too small or imbalanced.")
        return
    ...
    logging.info("Labeling and splitting completed successfully")
